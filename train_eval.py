...
class SelfProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        # file_path = os.path.join(data_dir, 'train_36471.csv')  # cnews.train.txt
        ...
        data = pd.read_csv('./data/train_36471.csv')
        # random.seed(0)
        # random.shuffle(data)
        ...
        #     split_line = line.strip().split("\t")
            ...
            # text_b = None
            # label = split_line[0]
            text_a=tokenization.convert_to_unicode(data['text'][index])
            text_b=None
    ...
    def get_dev_examples(self, data_dir):
        data=pd.read_csv('./data/valid_2000.csv')
        # file_path = os.path.join(data_dir, './data/val_2000.txt')
        # with open(file_path, 'r', encoding="utf-8") as f:
        #     reader = f.readlines()
    ...
    def get_labels(self):
        # print('get_labels:',sorted(set(self.labels), key=self.labels.index))
...
def save_PBmodel( num_labels):
    ...
    try:
        ...
        pb_file = os.path.join(arg_dic['pb_model_dir'], 'classification_model2.pb')
        graph = tf.Graph()
