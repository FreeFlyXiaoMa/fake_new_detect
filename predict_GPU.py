...
if __name__ == "__main__":
    ...
    toy=Bert_Class()
    #
    test=pd.read_csv('./data/test_stage1.csv')
    ids=test['id']
    text=test['text']
    # result_list1=[]
    ...
    #     result=toy.predict_on_ckpt(text[i])
    #     print('*'*20,result,'*'*20)
    #     result_list1.append(result)
    # df_re=pd.DataFrame()
    # df_re['id']=ids
    # df_re['label']=result_list1
    #
    # df_re.to_csv('./data/submit.csv',index=None)
    # t1=time.time()
    # print('预测耗时:',round(t1-t,3))

    result_list2=[]
    for j in range(len(text)):
        re=toy.predict_on_pb(text[j])
