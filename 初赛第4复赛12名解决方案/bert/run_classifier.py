...
flags.DEFINE_string(
    "train_test_set", "test2.csv",
    "The input data dir. Should contain the .tsv files (or other data files) "
    "for the task.")

flags.DEFINE_string(
    "test_set", "task1_test_stage.csv",
    "The input data dir. Should contain the .tsv files (or other data files) "
    "for the task.")

flags.DEFINE_string(
    "stack_set", "task1_test_stage.csv",
    "The input data dir. Should contain the .tsv files (or other data files) "
    "for the task.")

flags.DEFINE_string(
    "stack_dir", "/data/stacking",
    "The save data dir. Should contain the .tsv files (or other data files) "
    "for the task.")

flags.DEFINE_string(
    "bert_config_file", "/data/roberta/bert_config.json",
    #"bert_config_file", "/data/chinese_wwm_ext_base/bert_config.json",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", "detect", "The name of the task to train.")

flags.DEFINE_string("vocab_file",
                    "/data/roberta/vocab.txt",
                    # "/data/chinese_wwm_ext_base/vocab.txt",
...
flags.DEFINE_string(
    "test_save", "/data/res/roberta/base/results.csv",
    "The output directory where the model checkpoints will be written.")

## Other parameters

flags.DEFINE_string(
    "init_checkpoint", "/data/roberta/bert_model.ckpt",
    # "init_checkpoint", "/data/chinese_wwm_ext_base/bert_model.ckpt",
...
flags.DEFINE_bool("use_stack", False, "Whether to collect features for stacking.")
flags.DEFINE_bool("do_train", True, "Whether to run training.")
flags.DEFINE_bool("do_train_test", False, "Whether to run training test set.")
...
flags.DEFINE_integer("save_checkpoints_steps", 1080,
...
# TODO
def create_model(bert_config, mode, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    predict_labels = tf.squeeze(tf.argmax(log_probs, axis=-1, output_type=tf.int32))

    if mode == tf.estimator.ModeKeys.PREDICT:
        return (predict_labels, log_probs, logits)
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
        return (loss, per_example_loss, logits, probabilities, predict_labels)
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        (total_loss, per_example_loss, logits, probabilities) = create_model(
            bert_config, mode, input_ids, input_mask, segment_ids, label_ids,
            num_labels, use_one_hot_embeddings)

    if mode == tf.estimator.ModeKeys.EVAL:
        (total_loss, per_example_loss, logits, probabilities, predicted_labels) = create_model(
            bert_config, mode, input_ids, input_mask, segment_ids, label_ids,
            num_labels, use_one_hot_embeddings)
    if mode == tf.estimator.ModeKeys.PREDICT:
        (predicted_labels, probabilities, logits) = create_model(
            bert_config, mode, input_ids, input_mask, segment_ids, label_ids,
            num_labels, use_one_hot_embeddings)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      eval_metrics = (metric_fn,
                      [per_example_loss, label_ids, logits,predicted_labels,is_real_example])
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
      ...
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          predictions={"logits": logits, "label": predicted_labels},
          scaffold_fn=scaffold_fn)
    return output_spec
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    if FLAGS.do_train_test:
      ...
      num_train_test_steps = int(len(train_test_examples) / FLAGS.train_batch_size * 1)
      num_train_steps = num_train_steps + num_train_test_steps
  ...
  if FLAGS.do_train_test:
    ...
    train_test_examples = processor.get_train_test_examples(FLAGS.data_dir)
    num_train_test_steps = int(len(train_test_examples) / FLAGS.train_batch_size * 1)
    train_test_file = os.path.join(FLAGS.output_dir, "train_test.tf_record")
    file_based_convert_examples_to_features(train_test_examples, label_list, FLAGS.max_seq_length, tokenizer, train_test_file)
    tf.logging.info("***** Running training test set *****")
    tf.logging.info("  Num examples = %d", len(train_test_examples))
    tf.logging.info("  Batch size = %d", FLAGS.train_batch_size)
    tf.logging.info("  Num steps = %d", num_train_test_steps)
  ...
  if FLAGS.do_predict:
    ...
    predict_file = os.path.join(FLAGS.output_dir, "predict.test.tf_record")
